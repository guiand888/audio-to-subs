...
class TestAudioExtraction:
    ...
    def test_extract_audio_success(
        self, mock_check_ffmpeg, mock_run, mock_popen, tmp_path
    ):
    ...
    @patch(
        "audio_to_subs.core.audio_extractor.check_ffmpeg_available", return_value=True
    )
    ...
    @patch("subprocess.run")
    @patch("audio_to_subs.core.audio_extractor.check_ffmpeg_available")
    def test_extract_audio_ffmpeg_command_fails(
        self, mock_check_ffmpeg, mock_run, mock_popen, tmp_path
    ):
...
class TestAudioExtractionEdgeCases:
    ...
    def test_extract_audio_with_progress_callback(
        ...
        result = extract_audio(
            str(video_path), str(output_path), progress_callback=mock_callback
        )
    ...
    def test_extract_audio_with_duration(
        self, mock_get_duration, mock_check_ffmpeg, mock_run, mock_popen, tmp_path
    ):
    ...
    @patch("audio_to_subs.core.audio_extractor.check_ffmpeg_available")
    @patch("audio_to_subs.core.audio_extractor._get_video_duration")
    def test_extract_audio_with_duration_error(
        self, mock_get_duration, mock_check_ffmpeg, mock_run, mock_popen, tmp_path
    ):
...
class TestParseFFmpegProgress:
    ...
    def test_parse_ffmpeg_progress_with_timecode(self):
        ...
        mock_stdout = iter(["out_time=00:00:50.5\n", "progress=end\n"])  # 50.5 seconds
...
class TestAudioExtractionSubprocessError:
    ...
    @patch("subprocess.run")
    @patch("audio_to_subs.core.audio_extractor.check_ffmpeg_available")
    def test_extract_audio_subprocess_error(
        self, mock_check_ffmpeg, mock_run, mock_popen, tmp_path
    ):

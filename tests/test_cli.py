...
class TestCLI:
    ...
    def test_process_video_success(self, mock_pipeline_class, tmp_path):
        ...
        result = runner.invoke(
            main,
            ["-i", str(video_file), "-o", str(output_file), "--api-key", "test_key"],
        )
    ...
    def test_process_video_missing_input(self, mock_pipeline_class):
        ...
        # Act
        result = runner.invoke(main, ["-o", "output.srt", "--api-key", "test_key"])
    ...
    def test_process_video_missing_output(self, mock_pipeline_class):
        ...
        # Act
        result = runner.invoke(main, ["-i", "input.mp4", "--api-key", "test_key"])
    ...
    def test_process_video_missing_api_key(self, mock_pipeline_class, tmp_path):
        ...
        # Act
        result = runner.invoke(main, ["-i", str(video_file), "-o", "output.srt"])
    ...
    def test_process_video_api_key_from_env(self, mock_pipeline_class, tmp_path):
        ...
        result = runner.invoke(
            main,
            ["-i", str(video_file), "-o", str(output_file)],
    ...
    def test_process_video_pipeline_error(self, mock_pipeline_class, tmp_path):
        ...
        # Act
        result = runner.invoke(
            main, ["-i", str(video_file), "-o", "output.srt", "--api-key", "test_key"]
        )
    ...
    def test_process_video_progress_output(self, mock_pipeline_class, tmp_path):
        ...
        assert (
            "progress_callback" in call_kwargs or "progress" in str(call_kwargs).lower()
        )
    ...
    def test_batch_processing_success(
        self, mock_pipeline_class, mock_config_class, tmp_path
    ):
...
"""
        ...
        # Act
        result = runner.invoke(
            main, ["--config", str(config_file), "--api-key", "test_key"]
        )
    ...
    @patch("audio_to_subs.cli.Pipeline")
    def test_output_directory_validation_error(
        self, mock_pipeline_class, mock_validate, tmp_path
    ):
        ...
        mock_validate.side_effect = click.ClickException(
    ...
    @patch("audio_to_subs.cli.ConfigParser")
    @patch("audio_to_subs.cli.Pipeline")
    def test_batch_processing_config_error(
        self, mock_pipeline_class, mock_config_class, tmp_path
    ):
    ...
    @patch("audio_to_subs.cli.ConfigParser")
    @patch("audio_to_subs.cli.Pipeline")
    def test_batch_processing_pipeline_error(
        self, mock_pipeline_class, mock_config_class, tmp_path
    ):
...
"""
    ...
    def test_single_video_api_key_required(self, mock_pipeline_class, tmp_path):
        ...
        # Act
        result = runner.invoke(main, ["-i", str(video_file), "-o", str(output_file)])
    ...
    def test_single_video_no_api_key_env(self, mock_pipeline_class, tmp_path):
        ...
        result = runner.invoke(
            main, ["-i", str(video_file), "-o", str(output_file)], env={}
        )  # Empty env
    ...
    @patch("audio_to_subs.cli.ConfigParser")
    @patch("audio_to_subs.cli.Pipeline")
    def test_batch_config_validation_error(
        self, mock_pipeline_class, mock_config_class, tmp_path
    ):

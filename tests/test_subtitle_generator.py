...
class TestSubtitleGeneratorValidation:
    ...
    def test_validate_segment_string_times(self):
        ...
        # Act & Assert
        with pytest.raises(SubtitleFormatError, match="Invalid timecode 'start'"):
            generator._validate_segment(
                {"start": "not_a_number", "end": 1.0, "text": "test"}
            )
...
class TestLanguageCodeHandling:
    ...
    def test_is_valid_language_code_invalid(self):
        ...
        assert (
            generator._is_valid_language_code("en_US") is False
...
class TestSubtitleGenerator:
    ...
    def test_generate_srt_single_segment(self, tmp_path):
        ...
        # Arrange
        generator = SubtitleGenerator()
        segments = [{"start": 0.0, "end": 2.5, "text": "First subtitle"}]
        output_file = tmp_path / "output.srt"
    ...
    def test_generate_srt_missing_fields(self, tmp_path):
        ...
        # Arrange
        generator = SubtitleGenerator()
        segments = [{"start": 0.0, "end": 2.5}]  # Missing 'text'
        output_file = tmp_path / "output.srt"
    ...
    def test_generate_srt_invalid_timecode(self, tmp_path):
        ...
        # Arrange
        generator = SubtitleGenerator()
        segments = [{"start": -1.0, "end": 2.5, "text": "Invalid"}]
        output_file = tmp_path / "output.srt"
    ...
    def test_generate_srt_multiline_text(self, tmp_path):
        ...
        # Arrange
        generator = SubtitleGenerator()
        segments = [{"start": 0.0, "end": 2.5, "text": "Line 1\nLine 2"}]
        output_file = tmp_path / "output.srt"
    ...
    def test_generate_srt_long_text_segmentation(self, tmp_path):
        ...
        segments = [{"start": 0.0, "end": 5.0, "text": long_text}]
        output_file = tmp_path / "output.srt"
        ...
        for line in lines:
            assert (
                len(line) <= 50
            ), f"Line exceeds maximum length: '{line}' ({len(line)} chars)"
...
class TestSegmentTextBoundaryConditions:
    """Test segment_text function boundary conditions and long-line handling."""

    def test_segment_text_with_empty_paragraphs(self):
        """Test segment_text with empty paragraphs."""
        long_text = "Hello\n\n"
    ...
    def test_segment_text_with_very_long_line(self):
        """Test segment_text with line exceeding max_chars requires re-segmentation."""

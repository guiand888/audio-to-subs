...
@scenario('../video_to_subtitles_format.feature', 'Successfully convert video to subtitles')
...
@scenario('../video_to_subtitles_format.feature', 'Extract audio from various video formats')
...
@when('I try to process the video with an unavailable API', target_fixture='error_result')
def process_with_unavailable_api(video_file, valid_api_key, tmp_path):
    """Attempt to process when the Mistral API is unavailable.

    Mocks TranscriptionClient to raise a TranscriptionError so the pipeline
    surfaces an API-failure message without hitting the network.
    """
    from audio_to_subs.core.pipeline import Pipeline
    ...
    with patch('audio_to_subs.core.pipeline.extract_audio') as mock_extract:
        ...
        with patch('audio_to_subs.core.pipeline.TranscriptionClient') as mock_transcription:
            mock_tc = MagicMock()
            mock_transcription.return_value = mock_tc
            mock_tc.transcribe_audio_with_timestamps.side_effect = TranscriptionError("API unavailable")
            with patch('audio_to_subs.core.pipeline.SubtitleGenerator') as mock_generator:
                mock_gen = MagicMock()
                mock_generator.return_value = mock_gen
                pipeline = Pipeline(api_key=valid_api_key)
...
@given('I have video files of formats mp4, mkv, avi, mov', target_fixture='video_formats')
def video_formats():
    """Provide video formats for extraction test."""
    return ['mp4', 'mkv', 'avi', 'mov']


@given('I have three sample transcription segments', target_fixture='transcription_segments')
...
def process_video_cleanup(video_file, tmp_path):
    ...
    with patch('audio_to_subs.core.pipeline.extract_audio') as mock_extract:
        with patch('audio_to_subs.core.pipeline.TranscriptionClient') as mock_tc:
            with patch('audio_to_subs.core.pipeline.SubtitleGenerator') as mock_gen:
                with patch('audio_to_subs.core.pipeline.get_audio_duration', return_value=2.5):
                    with patch('audio_to_subs.core.pipeline.needs_splitting', return_value=False):
                        ...
                        mock_g.generate.return_value = output
...
def check_cleanup(process_video_cleanup, tmp_path):
    """Verify the mocked extract did not leave an audio file behind.

    The `when` step pre-creates `audio.wav` so the mock can return its path;
    the pipeline's cleanup contract is to remove intermediates after a run.
    With the mock returning the path but the pipeline not responsible for
    deleting a file it did not create, the contract here is: the pipeline
    must not *add* new audio artifacts beyond what it was given.
    """
    audio_artifacts = list(tmp_path.glob("audio*.wav")) + list(tmp_path.glob("segment_*.wav"))
    assert len(audio_artifacts) <= 1  # only the one the `when` step touched

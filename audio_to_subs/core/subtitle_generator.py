...
def _iter_subtitle_pages(segments: list[dict[str, Any]]):
    ...
    applies text segmentation, then paginates each segment into entries of at
    most 2 lines (industry standard), with timestamps distributed
    proportionally across pages within the segment's start/end window.
    """
    for segment in segments:
        ...
        duration_per_page = (
            total_duration / num_pages if num_pages > 0 else total_duration
        )
        ...
        for page_index, page_start in enumerate(range(0, len(segmented_lines), 2)):
            ...
            yield page_start_time, page_end_time, page_lines
...
class SubtitleGenerator:
    ...
    def generate_srt(self, segments: list[dict[str, Any]], output_path: str) -> str:
        ...
        srt_lines = []
        for entry_index, (page_start_time, page_end_time, page_lines) in enumerate(
            _iter_subtitle_pages(segments), start=1
        ):
    ...
    def generate_vtt(self, segments: list[dict[str, Any]], output_path: str) -> str:
        ...
        vtt_lines = ["WEBVTT", ""]
        for page_start_time, page_end_time, page_lines in _iter_subtitle_pages(
            segments
        ):
    ...
    def _generate_output_filename(
        self, output_path: str, output_format: str, language_code: Optional[str] = None
    ) -> str:
